...
| Hermes 侧 | `qty-assessment-workflow.md` 的创建模板 `tax_rate` 0 → 13（`create_quotation.py` 本就是 13） |

**生产验证**：存量 7 张全部回填为 13；导出 xlsx 备注行确认为「注：本报价单有效期 15 天，税率 13%。」；新建报价单默认 `tax_rate=13`（临时单建后即删）。

**测试:** backend **464 passed** (1 skipped, +3：默认 13 / 显式值可覆盖 / 导出备注含「税率 13%」)

> 注：前端仍**没有**税率设置入口（R21 有意移除）。当前税率只是导出文档上的说明文字，不参与任何金额计算；若以后要做成可编辑或参与含税计算，需另开一项（涉及 schema、导出、前端三处）。
