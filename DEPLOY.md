...
>
> ⛔ **保留策略必须按「名字」排快照，绝不能改成 `ls -1t`（按 mtime）**：`rsync -a` 含 `-t`，
> 会把**源目录的 mtime** 盖到目标快照目录上 → 同一源目录下建出来的快照 mtime 全一样，
> 按 mtime 排序是乱序，会删掉**最新那份**而留下旧的，且日志里的「份数」看着完全正常。
> 实现见脚本里的 `list_snapshots_desc()`，回归测试见 `backend/tests/test_round16.py`。
