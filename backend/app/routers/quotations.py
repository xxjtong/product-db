...
def create_quotation(data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    if solution_id:
        ...
        if sol:
            ...
            sol_items = db.query(SolutionItem).filter_by(solution_id=solution_id).all()
            for si in sol_items:
...
def update_quotation(quotation_id: int, data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
    qt = db.get(Quotation, quotation_id)
    if not qt:
        raise HTTPException(404, "Quotation not found")
...
def add_quotation_item(quotation_id: int, data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
    qt = db.get(Quotation, quotation_id)
    if not qt:
        raise HTTPException(404, "Quotation not found")
    ...
    qi = QuotationItem(
        ...
        unit_price=data.get("unit_price", float(prod.base_price or 0) if prod else 0),
...
def update_quotation_item(quotation_id: int, item_id: int, data: dict, db: Session = Depends(get_db), user=Depends(get_current_user)):
