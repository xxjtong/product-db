...
def batch_delete_solutions(data: BatchDeleteRequest, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    # 逐条走 ORM 删除，不能用 `db.query(...).delete()`：bulk DELETE 不经过 ORM 的
    # cascade 处理，`Solution.items` / `bom_snapshot` 的级联不会触发，会留下孤儿行 ——
    # 而 SQLite 的 rowid 会复用，孤儿子行可能被之后新建的方案「认领」。
    # （R57 起 SQLite 外键已强制开启，能兜住一部分；但 bulk delete 绕过的正是这一层，
    #   所以这里保持逐条删除。见 R41。）
